...
function decodeHtmlEntities(value) {
    return value.replace(/&(#(?:x[0-9a-f]+|[0-9]+)|[a-z]+);/gi, (match, raw) => {
        if (!raw.startsWith("#"))
            return HTML_ENTITIES[raw.toLowerCase()] ?? match;
 ...
 */
function readableEmailBody(value) {
 ...
 */
function documentTypeCode(value) {
    return text(value)
        .toUpperCase()
        .normalize("NFD")
        .replace(/[\u0300-\u036f]/g, "")
...
function normalizeRole(value) {
    ...
    if (["operador", "operator"].includes(role))
        return "operador";
    if (["visor", "viewer", "lectura"].includes(role))
        return "visor";
    return null;
}
...
async function requireCorreoAccess(data, context, allowedRoles) {
    ...
    if (!userBelongsToEmpresa(user, empresaId)) {
        throw new functions.https.HttpsError("permission-denied", "No perteneces a la empresa seleccionada.");
    }
    const role = await resolveCorreoRole(userSnap.id, user, empresaId);
    if (!role || !roleAllows(role, allowedRoles)) {
        throw new functions.https.HttpsError("permission-denied", "No tienes permisos para esta acción en Correo.");
    }
    return { empresaId, userId: userSnap.id, role };
}
function accountFromSnapshot(snap) {
...
function ruleFromSnapshot(snap) {
    const data = snap.data() ?? {};
...
function correoFilterIcon(value, category) {
    ...
    if (normalized.includes("tutela") ||
        ...
        return "⚖️";
    if (normalized.includes("requerimiento"))
        return "📌";
    if (normalized.includes("mejora"))
        return "📝";
    if (normalized.includes("pqr"))
        return "📬";
    if (normalized.includes("factura"))
        return "🧾";
    if (normalized.includes("urgente") || normalized.includes("alerta"))
        return "🚨";
    return "📩";
}
...
function encryptionKey() {
    ...
    if (!configured)
        throw new Error("CORREO_TOKEN_ENCRYPTION_KEY_MISSING");
    try {
...
function decrypt(value) {
    ...
    if (!ivRaw || !tagRaw || !encryptedRaw)
        throw new Error("CORREO_TOKEN_INVALID");
...
function gmailOAuthConfig(required = true) {
    ...
    const redirectUri = text(process.env.GMAIL_OAUTH_REDIRECT_URI) ||
...
function microsoftOAuthConfig(required = true) {
    ...
    const projectId = text(process.env.GCLOUD_PROJECT || process.env.GCP_PROJECT);
    const redirectUri = text(process.env.MICROSOFT_OAUTH_REDIRECT_URI) ||
        (projectId
...
function microsoftOAuthEndpoint(action) {
...
async function getGmailAccessToken(accountId) {
    ...
    if (!credential.exists)
        throw new Error("GMAIL_CREDENTIAL_NOT_FOUND");
    ...
    if (!accessToken)
        throw new Error("GMAIL_TOKEN_EMPTY");
    return accessToken;
}
async function getMicrosoftAccessToken(accountId) {
    const credential = await db()
        .collection("TBL_CORREO_CREDENCIALES")
        .doc(accountId)
        .get();
    if (!credential.exists)
        throw new Error("MICROSOFT_CREDENTIAL_NOT_FOUND");
    const refreshToken = decrypt(text(credential.get("refreshTokenEncrypted")));
    ...
    const response = await fetch(microsoftOAuthEndpoint("token"), {
        method: "POST",
        headers: { "Content-Type": "application/x-www-form-urlencoded" },
        body: body.toString(),
    });
    ...
    const accessToken = text(payload.access_token);
    if (!accessToken)
        throw new Error("MICROSOFT_TOKEN_EMPTY");
    return accessToken;
}
async function accountAccessToken(account) {
    return account.proveedor === "microsoft"
        ? getMicrosoftAccessToken(account.id)
        : getGmailAccessToken(account.id);
}
async function gmailRequest(accessToken, path, query) {
...
async function gmailPost(accessToken, path, body, method = "POST") {
...
async function graphRequest(accessToken, path, query) {
...
async function graphWrite(accessToken, path, body, method = "POST") {
...
function extractBody(payload) {
    ...
    const plainPart = parts.find((part) => normalizeText(part?.mimeType) === "text/plain");
    if (plainPart) {
...
function headersFromPayload(payload) {
    ...
    return Object.fromEntries(headers.map((header) => [normalizeText(header?.name), text(header?.value)]));
}
function collectAttachments(payload, output = []) {
    if (!payload || typeof payload !== "object")
        return output;
    ...
    const parts = Array.isArray(payload.parts) ? payload.parts : [];
    parts.forEach((part) => collectAttachments(part, output));
    return output;
}
...
function buildReplyMime(input) {
    ...
    const boundary = `capital_uspec_${(0, crypto_1.randomBytes)(18).toString("hex")}`;
    const parts = [
        ...headers,
    ...
    input.attachments.forEach((attachment) => {
        parts.push(`--${boundary}`, `Content-Type: ${attachment.mimeType}; name="${mimeHeader(attachment.filename)}"`, "Content-Transfer-Encoding: base64", `Content-Disposition: attachment; filename="${mimeHeader(attachment.filename)}"`, "", wrapBase64(attachment.bytes));
    });
    parts.push(`--${boundary}--`, "");
    return parts.join("\r\n");
}
async function getGmailMessage(accessToken, gmailMessageId) {
    ...
    const raw = await gmailRequest(accessToken, `/messages/${encodeURIComponent(gmailMessageId)}`, {
        format: "metadata",
    });
...
function parseMicrosoftMessage(raw) {
...
async function listMicrosoftMessages(accessToken) {
    const result = await graphRequest(accessToken, "/me/mailFolders/inbox/messages", {
        "$select": [
            "id",
            "conversationId",
            "from",
...
async function getGmailOutboundMessage(accessToken, messageId) {
    const raw = await gmailRequest(accessToken, `/messages/${encodeURIComponent(messageId)}`, { format: "metadata" });
    const headers = headersFromPayload(raw.payload);
    const rawDate = text(headers.date);
...
async function listGmailOutboundMessages(accessToken, since) {
    const after = Math.floor((since.toMillis() - GMAIL_SCAN_OVERLAP_MS) / 1000);
...
async function listMicrosoftOutboundMessages(accessToken, since) {
    ...
    const result = await graphRequest(accessToken, "/me/mailFolders/sentitems/messages", {
        "$select": [
            "id",
            "conversationId",
            "toRecipients",
...
function alertTemplateVariables(input) {
...
function friendlyDeliveryError(error) {
    const technical = error instanceof Error ? error.message : String(error);
    const normalized = normalizeText(technical);
    if (normalized.includes("session is not started") ||
...
function classifyCorreoAccountError(error) {
    const technical = error instanceof Error ? error.message : String(error);
    const normalized = normalizeText(technical);
    const requiresReconnect = normalized.includes("invalid_grant") ||
...
async function getRuleRecipients(listadoId, empresaId, moduleId) {
    if (!listadoId)
        return [];
    ...
    const modules = Array.isArray(list.get("modulos"))
        ...
        : [];
    if (modules.length && !modules.includes(normalizeText(moduleId)))
        return [];
    const recipients = Array.isArray(list.get("destinatarios")) ? list.get("destinatarios") : [];
    return recipients
        .filter((item) => item && item.activo !== false)
        .map((item) => ({ telefono: normalizePhone(item.telefono || item.phone || item), nombre: text(item.nombre) }))
        .filter((item) => item.telefono.length >= 8);
}
async function getActiveRules(empresaId) {
...
async function claimMessage(account, message) {
    const ref = db()
...
async function sendRuleAlerts(input) {
    const recipients = await getRuleRecipients(input.rule.listadoId, input.empresaId, "correo");
    ...
    let provider;
    try {
        provider = await (0, whatsapp_1.createWhatsAppProvider)(input.empresaId, "correo");
    }
    catch (error) {
 ...
 */
async function enviarWhatsAppAListado(input) {
    const recipients = await getRuleRecipients(input.listadoId, input.empresaId, input.moduleId || "correo");
    if (!recipients.length)
        return { sent: 0, failed: 0, skipped: 0 };
    const provider = await (0, whatsapp_1.createWhatsAppProvider)(input.empresaId, input.moduleId || "correo");
    const branding = await (0, notification_branding_1.loadCompanyNotificationBranding)(input.empresaId);
    ...
    let skipped = 0;
    for (const recipient of recipients) {
        const alertId = safeId(`${input.dedupeKey}_${sha256(recipient.telefono).slice(0, 16)}`);
        const alertRef = db().collection("TBL_CORREO_ALERTAS").doc(alertId);
        const shouldSend = await db().runTransaction(async (transaction) => {
            const existing = await transaction.get(alertRef);
            if (existing.exists)
            ...
            transaction.create(alertRef, {
                empresaId: input.empresaId,
                categoria: text(input.metadata?.categoria) ||
...
async function refreshCorreoMessageDeliveryState(messageId) {
    if (!messageId)
        return;
    ...
    await db().runTransaction(async (transaction) => {
        ...
        const accepted = states.filter((state) => state === "aceptado" || state === "enviado").length;
        const pending = states.some((state) => ["enviando", "reintentando"].includes(state));
...
async function retirePendingCorreoAlerts(empresaId) {
    const snap = await db().collection("TBL_CORREO_ALERTAS")
        .where("empresaId", "==", empresaId)
        .where("estado", "in", ["fallido", "enviando", "reintentando"]).get();
    let omitted = 0;
    for (const candidate of snap.docs.filter((doc) => text(doc.get("moduleId")) === "correo" || Boolean(text(doc.get("reglaId")))).slice(0, MAX_STALE_ALERTS_PER_RUN)) {
        const changed = await db().runTransaction(async (tx) => {
            const current = await tx.get(candidate.ref);
            if (!current.exists)
                return false;
            const state = text(current.get("estado"));
            if (!["fallido", "enviando", "reintentando"].includes(state))
                return false;
            const updated = timestampValue(current.get("updatedAt"))?.toMillis() ?? 0;
            if (state !== "fallido" && updated > Date.now() - STALE_ALERT_MS)
                return false;
...
async function alertOmissionForMessage(messageRef, receivedAt, empresaId) {
    ...
    if (expediente.exists && text(expediente.get("empresaId")) !== empresaId)
        return "empresa_incompatible";
    return (0, correo_alert_policy_1.correoAlertOmission)({
...
async function ensureAutomaticCorrespondence(input) {
    ...
    return db().runTransaction(async (transaction) => {
        ...
        transaction.create(expedienteRef, {
            ...
            tipoDocumentalSugerido: input.rule.tipoDocumental || input.rule.categoria || "General",
            reglaId: input.rule.id,
            reglaNombre: input.rule.nombre,
        ...
        transaction.create(eventRef, correspondenceEvent({
            ...
            userId: "sistema",
            detail: `Correo recibido en ${input.account.email || providerName}, ` +
...
async function findCorrespondenceForOutbound(account, message) {
    ...
    return candidates.find((doc) => !(0, correo_alert_policy_1.correspondenceHasResponse)(doc.data())) ?? candidates[0] ?? null;
}
async function claimOutboundMessage(account, message) {
    const ref = db()
        .collection("TBL_CORREO_MENSAJES")
...
async function syncOutboundCorrespondence(account, accessToken, message) {
    ...
    const wasResponded = (0, correo_alert_policy_1.correspondenceHasResponse)(expediente.data());
...
async function processAccount(account) {
    ...
    if (account.proveedor === "microsoft") {
        ...
        if (account.enviosBaselineCompletedAt && account.ultimaRevisionEnviosAt) {
            outboundMessages = await listMicrosoftOutboundMessages(accessToken, account.ultimaRevisionEnviosAt);
        }
    }
    else {
        ...
        const gmailQuery = [account.gmailQuery, after > 0 ? `after:${after}` : ""]
            .filter(Boolean)
            .join(" ");
        const list = await gmailRequest(accessToken, "/messages", { q: gmailQuery, maxResults: `${MAX_MESSAGES_PER_RUN}` });
        const refs = (list.messages ?? [])
            .map((item) => text(item.id))
            .filter(Boolean)
            .reverse();
        ...
        if (account.enviosBaselineCompletedAt && account.ultimaRevisionEnviosAt) {
            outboundMessages = await listGmailOutboundMessages(accessToken, account.ultimaRevisionEnviosAt);
        }
    }
    ...
    for (const message of messages) {
        ...
        if (result.rule.crearCorrespondencia) {
            try {
                ...
                if (correspondence.created)
                    expedientesCreados += 1;
            }
            catch (error) {
                console.error("[correo] prerradicación automática", messageRef.id, error);
                await messageRef.set({
                    correspondenciaError: "No fue posible crear el control documental automáticamente.",
                    updatedAt: admin.firestore.FieldValue.serverTimestamp(),
                }, { merge: true });
            }
        }
    ...
    await db().collection("TBL_CORREO_CUENTAS").doc(account.id).set({
        ...
        enviosBaselineCompletedAt: account.enviosBaselineCompletedAt || admin.firestore.FieldValue.serverTimestamp(),
        ultimoEstado: "ok",
...
async function processEmpresa(empresaId, accountId = "") {
    let accounts;
    if (accountId) {
    ...
    const totals = outcomes.reduce((acc, item) => ({
        ...
        expedientesCreados: acc.expedientesCreados + numberValue(item.expedientesCreados),
...
async function hydrateCorrespondenceFromGmail(input) {
    ...
    const raw = await gmailRequest(accessToken, `/messages/${encodeURIComponent(input.gmailMessageId)}`, { format: "full" });
    const headers = headersFromPayload(raw.payload);
    ...
    for (let index = 0; index < attachments.length; index += 1) {
        ...
        const result = await gmailRequest(accessToken, `/messages/${encodeURIComponent(input.gmailMessageId)}/attachments/${encodeURIComponent(attachment.attachmentId)}`);
        ...
        const downloadToken = (0, crypto_1.randomBytes)(24).toString("hex");
...
async function hydrateCorrespondenceFromMicrosoft(input) {
    ...
    const raw = await graphRequest(accessToken, `/me/messages/${encodeURIComponent(input.microsoftMessageId)}`, { "$select": "body,toRecipients,internetMessageId,hasAttachments" });
    const attachmentResult = raw.hasAttachments
        ? await graphRequest(accessToken, `/me/messages/${encodeURIComponent(input.microsoftMessageId)}/attachments`)
        : { value: [] };
    const attachments = (attachmentResult.value ?? []).filter((item) => text(item?.["@odata.type"]).endsWith("fileAttachment") &&
...
async function hydrateCorrespondence(input) {
    const account = await db()
        ...
        .get();
    if (!account.exists)
        throw new Error("CORREO_ACCOUNT_NOT_FOUND");
...
function responseAttachmentRows(value) {
    if (!Array.isArray(value))
        return [];
    return value
...
async function buildExpedienteResponse(input) {
    ...
    if (!to || !subject || !body) {
        throw new functions.https.HttpsError("failed-precondition", "Completa destinatario, asunto y respuesta antes de guardar en Gmail.");
    }
    ...
    for (const row of rows) {
        ...
        if (totalBytes > 20 * 1024 * 1024) {
            throw new functions.https.HttpsError("failed-precondition", "Los adjuntos superan 20 MB. Reduce el tamaño antes de guardar o enviar.");
        }
...
async function upsertGmailDraft(input) {
    ...
    if (existingDraftId) {
        try {
            draft = await gmailPost(input.accessToken, `/drafts/${encodeURIComponent(existingDraftId)}`, { id: existingDraftId, message }, "PUT");
        }
        catch (error) {
            const detail = error instanceof Error ? error.message : String(error);
            if (!detail.startsWith("GMAIL_API_404:"))
                throw error;
...
async function buildMicrosoftResponse(input) {
    ...
    const body = text(input.doc.get("respuestaCuerpo"));
    if (!to || !subject || !body) {
        throw new functions.https.HttpsError("failed-precondition", "Completa destinatario, asunto y respuesta antes de guardar el borrador.");
    }
    const rows = responseAttachmentRows(input.doc.get("adjuntosRespuesta"));
    const bucket = admin.storage().bucket();
    let totalBytes = 0;
    ...
    const recipients = (value) => textList(value)
...
async function upsertMicrosoftDraft(input) {
    ...
    if (!draftId) {
        const sourceId = text(input.doc.get("providerMessageId") || input.doc.get("microsoftMessageId"));
        const draft = sourceId
            ? await graphWrite(input.accessToken, `/me/messages/${encodeURIComponent(sourceId)}/createReply`, {})
            : await graphWrite(input.accessToken, "/me/messages", response.message);
        draftId = text(draft.id);
        threadId = text(draft.conversationId) || threadId;
    }
    if (!draftId)
        throw new Error("MICROSOFT_DRAFT_ID_EMPTY");
    await graphWrite(input.accessToken, `/me/messages/${encodeURIComponent(draftId)}`, response.message, "PATCH");
    const currentAttachments = await graphRequest(input.accessToken, `/me/messages/${encodeURIComponent(draftId)}/attachments`, { "$select": "id" });
    for (const attachment of currentAttachments.value ?? []) {
        const id = text(attachment.id);
        if (id) {
            await graphWrite(input.accessToken, `/me/messages/${encodeURIComponent(draftId)}/attachments/${encodeURIComponent(id)}`, undefined, "DELETE");
        }
    }
    for (const attachment of response.attachments) {
        await graphWrite(input.accessToken, `/me/messages/${encodeURIComponent(draftId)}/attachments`, attachment);
    }
    return { id: draftId, threadId, to: response.to };
}
function gmailPermissionError(error) {
    const detail = error instanceof Error ? error.message : String(error);
    const normalized = normalizeText(detail);
    if (normalized.includes("access_token_scope_insufficient") ||
        normalized.includes("insufficient authentication scopes") ||
        normalized.includes("insufficient permission")) {
        return new functions.https.HttpsError("failed-precondition", "Reconecta este buzón Gmail desde Administración para autorizar Borradores y Envíos.");
    }
    return null;
}
function microsoftPermissionError(error) {
    const detail = error instanceof Error ? error.message : String(error);
    const normalized = normalizeText(detail);
    if (normalized.includes("erroraccessdenied") ||
        normalized.includes("insufficient privileges") ||
        normalized.includes("invalid_grant") ||
        normalized.includes("token_refresh")) {
        return new functions.https.HttpsError("failed-precondition", "Reconecta este buzón Microsoft desde Administración para autorizar Borradores y Envíos.");
    }
    return null;
}
function providerPermissionError(provider, error) {
 ...
 */
exports.correoCrearExpediente = functions
    .region(REGION)
    ...
    if (!correoMensajeId || !responsableId) {
        throw new functions.https.HttpsError("invalid-argument", "El correo y el responsable son obligatorios.");
    }
    ...
    const created = await db().runTransaction(async (transaction) => {
        ...
        transaction.create(expedienteRef, {
            ...
            providerMessageId: text(message.get("providerMessageId") || message.get("gmailMessageId")),
            providerThreadId: text(message.get("providerThreadId") || message.get("gmailThreadId")),
...
exports.gdAsignarExpediente = functions
    .region(REGION)
    .runWith({ timeoutSeconds: 120, memory: "512MB" })
    .https.onCall(async (data, context) => {
    ...
    if (!expedienteId ||
        ...
        throw new functions.https.HttpsError("invalid-argument", "Tipo documental, responsable y fecha límite son obligatorios.");
    }
    ...
    if (!responsable?.exists ||
        ...
        throw new functions.https.HttpsError("failed-precondition", "El responsable no pertenece a la empresa activa.");
    }
    ...
    await db().runTransaction(async (transaction) => {
        ...
        const taskPayload = {
            schemaVersion: 2,
            titulo: `Responder ${internalCode || text(current.get("radicado"))}: ` +
                `${text(current.get("asunto"))}`,
            descripcion: `Correspondencia ${tipoDocumental} recibida de ${text(current.get("remitente"))}. ` +
        ...
        transaction.create(eventRef, correspondenceEvent({
            ...
            userId: caller.userId,
            detail: `${tipoDocumental}${internalCode ? ` (${internalCode})` : ""} ` +
    ...
    if (!["disponible", "preparando"].includes(currentState)) {
        ...
        try {
            await hydrateCorrespondence({
                ...
                providerMessageId: text(initial.get("providerMessageId") || initial.get("gmailMessageId")),
            });
        }
        catch (error) {
            ...
            await expedienteRef.set({
                entradaEstado: "error",
                entradaError: "No fue posible descargar el correo y sus adjuntos. Usa Reintentar.",
                updatedAt: admin.firestore.FieldValue.serverTimestamp(),
            }, { merge: true });
        }
    }
 ...
 */
exports.gdCodificarExpedientesHistoricos = functions
    .region(REGION)
    ...
    for (const doc of typeSnap.docs) {
        ...
        for (const name of names) {
            ...
            if (normalized)
                codesByName.set(normalized, code);
        }
    }
    ...
    for (const expediente of expedienteSnap.docs) {
        ...
        const originalDate = timestampValue(expediente.get("clasificadoAt")) ??
        ...
        try {
            const assignedSequence = await db().runTransaction(async (transaction) => {
                ...
                if (!current.exists || text(current.get("codigoInterno")))
                    return null;
                const observed = maxByGroup.get(groupKey) ?? 0;
                const sequence = Math.max(numberValue(counter.get("ultimo")), observed) + 1;
                const internalCode = `${typeCode}${dayStamp}-${sequence.toString().padStart(3, "0")}`;
                transaction.set(counterRef, {
                    empresaId: caller.empresaId,
...
exports.gdRegistrarRespuestaExterna = functions
    .region(REGION)
    .https.onCall(async (data, context) => {
    ...
    const checkAccess = (doc) => {
        if (!doc.exists || text(doc.get("empresaId")) !== caller.empresaId) {
            throw new functions.https.HttpsError("not-found", "Expediente no encontrado.");
        }
        if (!(0, correo_alert_policy_1.canRegisterExternalResponse)({
    ...
    checkAccess(current);
    if ((0, correo_alert_policy_1.correspondenceHasResponse)(current.data() ?? {}))
    ...
    const attachment = {
        ...
        downloadUrl: await (0, storage_1.getDownloadURL)(file),
    ...
    return db().runTransaction(async (tx) => {
        ...
        checkAccess(doc);
        if ((0, correo_alert_policy_1.correspondenceHasResponse)(doc.data() ?? {}))
            return { ok: true, alreadyRegistered: true };
...
exports.gdTerminarExpediente = functions
    .region(REGION)
    .https.onCall(async (data, context) => {
    const caller = await requireCorreoAccess(data, context, ["operador"]);
    const expedienteId = text(data?.expedienteId);
    if (!expedienteId || expedienteId.includes("/")) {
        throw new functions.https.HttpsError("invalid-argument", "El expediente es obligatorio.");
    }
    const expedienteRef = db().collection("TBL_GD_EXPEDIENTES").doc(expedienteId);
    ...
    if (!esAdministrador && (!responsableId || responsableId !== caller.userId)) {
        throw new functions.https.HttpsError("permission-denied", "Solo el responsable asignado o un administrador del módulo puede " +
    ...
    const validado = (0, gd_cierre_policy_1.validateGdCierre)({
        motivo: data?.motivoCierre,
        justificacion: data?.justificacion,
        tieneRespuesta: (0, correo_alert_policy_1.correspondenceHasResponse)(expediente.data() ?? {}),
    });
    ...
    const detalleCierre = (0, gd_cierre_policy_1.gdCierreEventDetail)({ cerradoPorTercero, cierre });
    const batch = db().batch();
...
exports.correoPrepararExpediente = functions
    .region(REGION)
    .runWith({ timeoutSeconds: 120, memory: "512MB" })
    .https.onCall(async (data, context) => {
    const caller = await requireCorreoAccess(data, context, ["operador"]);
    ...
    await hydrateCorrespondence({
        ...
        providerMessageId: text(doc.get("providerMessageId") || doc.get("gmailMessageId")),
    });
    return { ok: true };
});
/** Aprueba o devuelve una respuesta cuando el expediente exige revisión. */
exports.gdRevisarRespuesta = functions
    .region(REGION)
    .https.onCall(async (data, context) => {
    const caller = await requireCorreoAccess(data, context, ["operador"]);
    const expedienteId = text(data?.expedienteId);
...
exports.correoGuardarBorradorGmail = functions
    .region(REGION)
    .runWith({ timeoutSeconds: 120, memory: "512MB" })
    .https.onCall(async (data, context) => {
    const caller = await requireCorreoAccess(data, context, ["operador"]);
    ...
    if ((0, correo_alert_policy_1.correspondenceHasResponse)(doc.data() ?? {})) {
        throw new functions.https.HttpsError("failed-precondition", "La respuesta ya fue enviada y no puede volver a guardarse como borrador.");
    }
...
exports.correoEnviarRespuesta = functions
    .region(REGION)
    .runWith({ timeoutSeconds: 120, memory: "512MB" })
    .https.onCall(async (data, context) => {
    const caller = await requireCorreoAccess(data, context, ["operador"]);
    ...
    if (doc.get("requiereAprobacion") === true && text(doc.get("aprobacionEstado")) !== "aprobada") {
        throw new functions.https.HttpsError("failed-precondition", "La respuesta requiere aprobación antes de enviarse.");
    }
    const accountId = text(doc.get("cuentaId"));
    const account = await db().collection("TBL_CORREO_CUENTAS").doc(accountId).get();
    if (!account.exists || text(account.get("empresaId")) !== caller.empresaId) {
    ...
    try {
        if (provider === "microsoft") {
            ...
            draftId = draft.id;
            to = draft.to;
            await graphWrite(accessToken, `/me/messages/${encodeURIComponent(draft.id)}/send`, {});
            ...
            const draft = await upsertGmailDraft({ accessToken, doc, account });
            draftId = draft.id;
            sent = await gmailPost(accessToken, "/drafts/send", { id: draft.id });
        }
    }
    catch (error) {
        const permissionError = providerPermissionError(provider, error);
...
exports.correoGmailAuthorize = functions
    .region(REGION)
    .https.onCall(async (data, context) => {
    const caller = await requireCorreoAccess(data, context, ["administrador"]);
...
exports.correoGmailCallback = functions
    .region(REGION)
    .https.onRequest(async (req, res) => {
    const show = (title, body, status = 200) => {
        res.status(status).set("Content-Type", "text/html; charset=utf-8").send(`<!doctype html><html><body style="font-family:Arial;padding:32px;color:#1e293b"><h2>${title}</h2><p>${body}</p><p>Puedes cerrar esta ventana y volver a la aplicación.</p></body></html>`);
    };
    try {
        ...
        if (oauthError)
            return show("Autorización cancelada", "Google no autorizó el acceso al buzón.", 400);
        if (!code || !state)
        ...
        if (!exchange.ok)
            throw new Error(`GMAIL_OAUTH_EXCHANGE_${exchange.status}`);
        const token = (await exchange.json());
        const accessToken = text(token.access_token);
        const refreshToken = text(token.refresh_token);
        if (!accessToken || !refreshToken)
            throw new Error("GMAIL_OAUTH_REFRESH_TOKEN_MISSING");
        ...
        const refreshTokenExpiresAt = refreshTokenExpiresIn > 0
            ? admin.firestore.Timestamp.fromMillis(Date.now() + refreshTokenExpiresIn * 1000)
...
exports.correoMicrosoftAuthorize = functions
    .region(REGION)
    .https.onCall(async (data, context) => {
    const caller = await requireCorreoAccess(data, context, ["administrador"]);
    const accountId = text(data?.cuentaId || data?.accountId);
    ...
    if (!expectedEmail) {
        throw new functions.https.HttpsError("failed-precondition", "Indica el correo exacto de Microsoft 365 antes de conectarlo.");
    }
    const config = microsoftOAuthConfig();
    const state = (0, crypto_1.randomBytes)(32).toString("base64url");
    await db().collection("TBL_CORREO_OAUTH_STATES").doc(state).set({
...
exports.correoMicrosoftCallback = functions
    .region(REGION)
    .https.onRequest(async (req, res) => {
    const show = (title, body, status = 200) => {
        res.status(status).set("Content-Type", "text/html; charset=utf-8").send(`<!doctype html><html><body style="font-family:Arial;padding:32px;color:#1e293b"><h2>${title}</h2><p>${body}</p><p>Puedes cerrar esta ventana y volver a la aplicación.</p></body></html>`);
    ...
    try {
        ...
        if (adminConsent) {
            return show("Aprobación administrativa completada", "Integra360 Correo fue autorizado para esta organización. " +
                "Ahora conecta el buzón desde Administración > Correo.");
        }
        if (oauthError) {
            return show("Autorización cancelada", "Microsoft no autorizó el acceso al buzón.", 400);
        }
        if (!code || !state) {
            return show("Enlace inválido", "Faltan parámetros de autorización.", 400);
        }
        ...
        const exchange = await fetch(microsoftOAuthEndpoint("token"), {
            method: "POST",
            headers: { "Content-Type": "application/x-www-form-urlencoded" },
            body: body.toString(),
        });
        ...
        if (expectedEmail && !authorizedEmails.includes(expectedEmail)) {
            ...
            await accountRef.set({
                ...
                ultimoEstado: "error",
                ultimoError: "La cuenta autorizada no coincide con el correo indicado.",
                updatedAt: admin.firestore.FieldValue.serverTimestamp(),
            }, { merge: true });
            return show("Cuenta diferente", "Selecciona en Microsoft el mismo correo que indicaste en Integra360.", 400);
        }
        await db().collection("TBL_CORREO_CREDENCIALES").doc(accountId).set({
            provider: "microsoft",
            refreshTokenEncrypted: encrypt(refreshToken),
        ...
        await stateRef.delete();
        return show("Microsoft 365 conectado", "El buzón fue autorizado correctamente.");
    }
    catch (error) {
        console.error("[correo] Microsoft OAuth callback error", error);
        return show("No fue posible conectar Microsoft 365", "Revisa la configuración OAuth e inténtalo de nuevo.", 500);
    }
});
/** Procesamiento manual desde Flutter. */
exports.correoProcesar = functions
    .region(REGION)
...
exports.correoProcesarHttp = functions
    .region(REGION)
    .https.onRequest(async (req, res) => {
    if (req.method !== "POST")
...
exports.correoProcesarProgramado = functions
    .region(REGION)
 ...
 */
exports.correoProbarRegla = functions
    .region(REGION)
    .https.onCall(async (data, context) => {
    const caller = await requireCorreoAccess(data, context, ["administrador"]);
    let rule = null;
    ...
    if (ruleId) {
        ...
        rule = ruleFromSnapshot(ruleDoc);
    }
    else if (data?.regla && typeof data.regla === "object") {
        const fake = { id: "prueba", data: () => data.regla };
        rule = ruleFromSnapshot(fake);
    }
    if (!rule)
        throw new functions.https.HttpsError("invalid-argument", "reglaId o regla requerido.");
...
exports.correoProbarWhatsApp = functions
    ...
    const provider = await (0, whatsapp_1.createWhatsAppProvider)(caller.empresaId, "correo");
    const verification = provider.checkRecipient
...
exports.correoEstadoIntegracion = functions
    ...
    const whatsappState = await (0, whatsapp_1.getWhatsAppPublicState)(caller.empresaId, "correo");
    return {
        gmailOAuthConfigured: Boolean(gmail.clientId && gmail.clientSecret && gmail.redirectUri),
        gmailRedirectUri: gmail.redirectUri || null,
        microsoftOAuthConfigured: Boolean(microsoft.clientId &&
