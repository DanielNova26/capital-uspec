...
exports.comprasNotificarNuevoProveedorWhatsApp = exports.comprasNotificarRecepcionCalidad = void 0;
...
exports.comprasNotificarRecepcionCalidad = functions
    .region(REGION)
    ...
    for (const role of roles.docs) {
        if (normalize(role.get("rol")) !== "calidad")
            continue;
    ...
    const branding = await (0, notification_branding_1.loadCompanyNotificationBranding)(empresaId);
...
async function resolveListadoId(empresaId, configuredId) {
    if (configuredId)
        return configuredId;
...
function comprasRoleRecipient(role) {
...
async function notifyInAppNewSupplier(input) {
    ...
    if (!recipients.size)
        return { estado: "sin_destinatarios", enviados: 0 };
    const notificationId = `compras_nuevo_proveedor_${input.proveedorId}`;
    const title = `${branding.emoji} Nuevo proveedor registrado`;
    const description = `${input.razonSocial} (${input.nit}) fue registrado y su documentación ` +
    ...
    await Promise.all([...recipients].map(async (userId) => {
        const parent = admin
            ...
            .doc(userId);
        await parent.set({ updatedAt: admin.firestore.FieldValue.serverTimestamp() }, { merge: true });
 ...
 */
exports.comprasNotificarNuevoProveedorWhatsApp = functions
    .region(REGION)
    .firestore.document("TBL_COMPRAS_PROVEEDORES/{proveedorId}")
    .onCreate(async (snapshot, context) => {
    const data = snapshot.data();
    if (data.notificarWhatsAppNuevoProveedor !== true)
        return null;
    const empresaId = text(data.empresaId);
    ...
    const config = await admin
        ...
        .doc(empresaId)
        .get();
    const centralListId = await (0, whatsapp_1.getWhatsAppRouteListId)(empresaId, "compras_nuevo_proveedor");
    const listadoId = await resolveListadoId(empresaId, centralListId || text(config.get("whatsappNuevoProveedorListadoId")));
    if (!listadoId) {
        await snapshot.ref.set({
            notificarWhatsAppNuevoProveedor: false,
            whatsappNuevoProveedor: {
                estado: "sin_configuracion",
                error: "Asigna una lista en Administración > WhatsApp para el evento de proveedor nuevo.",
                updatedAt: admin.firestore.FieldValue.serverTimestamp(),
            },
    ...
    const whatsAppState = await (0, whatsapp_1.getWhatsAppPublicState)(empresaId, "compras");
    if (!whatsAppState.enabled || !whatsAppState.moduleEnabled) {
        await snapshot.ref.set({
            notificarWhatsAppNuevoProveedor: false,
            whatsappNuevoProveedor: {
                estado: "deshabilitado",
                error: "Los avisos de Compras están deshabilitados desde Administración > WhatsApp.",
                updatedAt: admin.firestore.FieldValue.serverTimestamp(),
            },
            notificacionCampanaNuevoProveedor: {
                ...campana,
    ...
    const mensaje = [
        ...
        .join("\n");
    try {
        const result = await (0, correo_1.enviarWhatsAppAListado)({
            empresaId,
            listadoId,
            mensaje,
