...
exports.rutasMovilidadMedirAhora = exports.rutasMovilidadTick = exports.ETIQUETA_INCIDENTE = void 0;
 ...
 */
async function fetchConReintentos(url, init, etiqueta) {
    ...
    for (let intento = 1; intento <= MAX_INTENTOS; intento++) {
        try {
            ...
            if (resp.ok)
                return { ok: true, status: resp.status, text, error: "" };
            const recuperable = resp.status === 429 || resp.status === 403 || resp.status >= 500;
            ultimo = {
                ok: false,
                status: resp.status,
                text,
            ...
            if (!recuperable || intento === MAX_INTENTOS)
                return ultimo;
            const espera = 400 * Math.pow(3, intento - 1); // 400ms, 1.2s
            console.warn(`[movilidad] ${etiqueta} HTTP ${resp.status}, reintento ` +
 ...
 */
function clasificarRiesgo(min) {
    if (min <= 60)
        return "bajo";
    if (min <= 90)
        return "medio";
    if (min <= 120)
        return "alto_controlado";
    return "critico";
}
/**
 ...
 */
function clasificarTrafico(conTrafico, sinTrafico) {
    if (!sinTrafico || sinTrafico <= 0)
        return "medio";
 ...
 */
function escenarioAutomatico(weekday, minutosDia) {
    if (weekday >= 6)
        return "fin_semana";
    if (minutosDia < 9 * 60)
        return "pico_manana";
    if (minutosDia < 11 * 60 + 30)
        return "valle";
    if (minutosDia <= 14 * 60)
        return "medio_dia";
    if (minutosDia < 16 * 60 + 30)
        return "valle";
    if (minutosDia <= 20 * 60)
        return "pico_tarde";
    return "valle";
 ...
 */
async function obtenerIncidentes(bbox, apiKey) {
    ...
    try {
        ...
        const lista = Array.isArray(json.incidents) ? json.incidents : [];
        return lista.map((i) => {
 ...
 */
const PMT_SERVICE = "https://sig.simur.gov.co/arcgis/rest/services/PMT/" +
 ...
 */
async function obtenerObrasPmt(bbox) {
    const salida = [];
    for (const capa of PMT_CAPAS) {
        ...
        const r = await fetchConReintentos(`${PMT_SERVICE}/${capa.id}/query?${params.toString()}`, {}, `pmt-capa-${capa.id}`);
        if (!r.ok) {
 ...
 */
function tocaRuta(indice, muestras) {
    return muestras.some((m) => indice.has(`${Math.floor(m[0] / CELDA_GRADOS)}:${Math.floor(m[1] / CELDA_GRADOS)}`));
}
/**
 ...
 */
function resumirObrasPmt(obras, indice) {
    ...
    for (const o of enRuta) {
        ...
        if (!porRadicado.has(clave))
            porRadicado.set(clave, o);
    }
    const unicas = [...porRadicado.values()];
    return {
        total: unicas.length,
        tramosAfectados: enRuta.length,
        cierres: unicas.filter((o) => o.tipoAfectacion.toUpperCase().includes("CIERRE")).length,
 ...
 */
function incidentesEnRuta(indice, incidentes) {
 ...
 * @return {Record<string, unknown>} Conteos + detalle recortado.
 */
function resumirIncidentes(lista) {
    const cuenta = (c) => lista.filter((i) => i.categoria === c).length;
 ...
 */
async function medirGoogle(origen, destino, apiKey, salidaMs) {
    ...
    const departureTime = new Date(salidaMs && salidaMs > base0 ? salidaMs : base0).toISOString();
    const body = {
        origin: {
    ...
    try {
        const r = await fetchConReintentos("https://routes.googleapis.com/directions/v2:computeRoutes", {
            method: "POST",
            headers: {
        ...
        const rutas = Array.isArray(json.routes) ? json.routes : [];
        if (rutas.length === 0) {
        ...
        if (encoded)
            base.puntos = decodificarPolyline(encoded);
        return base;
    }
    catch (e) {
 ...
 * @return {Promise<ResultadoApi>} Resultado normalizado + JSON crudo.
 */
async function medirTomTom(origen, destino, apiKey, salidaMs) {
    const path = `${origen.lat},${origen.lng}:${destino.lat},${destino.lng}`;
    ...
    try {
        ...
        base.rutaAlterna = alt ?
            `Alterna (${Math.round((Number(alt.travelTimeInSeconds) || 0) / 60)} ` +
                "min)" :
            "";
        const legs = Array.isArray(rutas[0].legs) ? rutas[0].legs : [];
        base.puntos = legs.flatMap((l) => (Array.isArray(l.points) ? l.points : []).map((p) => [
            Number(p.latitude),
            Number(p.longitude),
        ]));
        return base;
 ...
 */
async function ejecutarCorrida(op) {
    ...
    if (!configSnap.exists) {
        throw new Error("La empresa no tiene configuración del estudio de movilidad " +
    ...
    const keys = {
        google: String(config.apiKeyGoogle ?? config.apiKey ?? "").trim() ||
            String(process.env.MOVILIDAD_GOOGLE_API_KEY ?? "").trim(),
        tomtom: String(config.apiKeyTomtom ?? "").trim() ||
            String(process.env.MOVILIDAD_TOMTOM_API_KEY ?? "").trim(),
    };
    if (!keys[fuente]) {
        throw new Error(`No hay API key configurada para la fuente "${fuente}" ` +
    ...
    if (config.compararFuentes === true && !comparar) {
        console.warn(`[movilidad] comparativo pedido pero sin key de "${secundaria}": ` +
    ...
    const alertaCedulas = Array.isArray(config.alertaCedulas) ?
        config.alertaCedulas.map((c) => String(c)).filter(Boolean) :
        [];
    ...
    const rutas = rutasSnap.docs
        .map((d) => {
        const m = d.data();
        const paradas = (m.stops ?? [])
            .map((s, i) => ({
            id: String(s?.establecimientoId ?? s?.id ?? ""),
            nombre: String(s?.nombre ?? s?.name ?? ""),
            direccion: String((String(s?.direccionLimpia ?? "").trim() || s?.direccionRaw) ?? ""),
            lat: Number(s?.lat),
            lng: Number(s?.lng),
        ...
        return {
            ...
            numero: Number(String(m.codigo ?? "").replace(/[^0-9]/g, "")) || 9999,
            paradas,
        };
    })
    ...
    if (rutas.length === 0) {
        throw new Error("No hay rutas activas con paradas geocodificadas. Abre Estudio " +
    ...
    if (keys.tomtom) {
        ...
        console.log(`[movilidad] incidentes viales vigentes: ${incidentes.length}`);
    }
    ...
    async function medirRuta(ruta) {
        ...
        for (let idx = 0; idx < ruta.paradas.length; idx++) {
            ...
            const salidaMs = inicioCorridaMs +
                (acum[fuente].min + minutosPorParada * idx) * 60000;
            const resultados = await Promise.all(fuentesAMedir.map(async (fuenteCfg) => {
                const r = fuenteCfg === "tomtom" ?
                    await medirTomTom({ lat: desde.lat, lng: desde.lng }, { lat: parada.lat, lng: parada.lng }, keys.tomtom, salidaMs) :
                    await medirGoogle({ lat: desde.lat, lng: desde.lng }, { lat: parada.lat, lng: parada.lng }, keys.google, salidaMs);
                return { fuenteCfg, r };
            }));
            for (const { fuenteCfg, r } of resultados) {
                ...
                const enRutaMin = Math.round((acumMin + minutosPorParada * idx) * 10) / 10;
                ...
                const obrasTr = resumenRuta.obras;
                const obrasOf = resumenObras.total;
                if (r.ok && (obrasTr > 0 || obrasOf > 0)) {
                    const partes = [];
                    if (obrasOf > 0) {
                        partes.push(`${obrasOf} obra(s) autorizada(s) por la SDM (PMT) sobre este ` +
                            "tramo");
                    }
                    if (obrasTr > 0) {
                ...
                try {
                    ...
                    fallidos++;
                    console.error("[movilidad] no se pudo guardar medición de " +
                        `${parada.nombre} (${ruta.codigo}, ${fuenteCfg})`, e);
                }
            }
        ...
        for (const cedula of alertaCedulas) {
            try {
                await db()
                    ...
                    title: `Estudio movilidad: ${alertadas.length} ruta(s) en alerta ` +
                        `(≥ ${umbralAlertaMin} min)`,
    ...
    console.log("[movilidad] corrida", JSON.stringify({
        runId: op.runRef.id,
        empresaId: op.empresaId,
        tipo: op.tipo,
        escenario: op.escenario,
...
exports.rutasMovilidadTick = functions
    .region(REGION)
    ...
    for (const doc of horariosSnap.docs) {
        ...
        try {
            ...
            await runRef.set({ estado: "error", errorMsg: String(e) }, { merge: true });
        }
    }
    return null;
});
// ── 2) Callable: medición manual desde la app ───────────────────────────────
exports.rutasMovilidadMedirAhora = functions
    .region(REGION)
    .runWith({ timeoutSeconds: 540, memory: "512MB" })
    ...
    const ahora = bogotaNow();
    const escenario = escenarioAutomatico(isoWeekday(ahora), ahora.getUTCHours() * 60 + ahora.getUTCMinutes());
