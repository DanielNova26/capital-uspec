...
function normalizedMetaTemplates(value) {
    if (!value || typeof value !== "object" || Array.isArray(value))
        return {};
    const result = {};
    for (const [rawKey, rawValue] of Object.entries(value)) {
...
function envConfig(empresaId) {
    ...
    const cloudProvider = ["whatsapp_cloud", "cloud", "whatsappcloud"].includes(provider);
    return {
        empresaId,
        provider,
        baseUrl: text(process.env.WHATSAPP_API_BASE_URL).replace(/\/+$/, ""),
        apiKey: text(cloudProvider
...
async function loadRuntimeConfig(empresaId) {
    ...
    const forceEnvironmentCredentials = normalize(process.env.WHATSAPP_FORCE_ENV_CONFIG) === "true";
    ...
    return {
        ...
        baseUrl: text(forceEnvironmentCredentials ? fallback.baseUrl : data.baseUrl || fallback.baseUrl).replace(/\/+$/, ""),
        apiKey: forceEnvironmentCredentials ? fallback.apiKey : apiKey,
        sessionId: text(forceEnvironmentCredentials ? fallback.sessionId : data.sessionId || fallback.sessionId),
        wabaId: text(forceEnvironmentCredentials ? fallback.wabaId : data.wabaId || fallback.wabaId),
...
class WhatsAppCloudProvider {
    ...
    async send(input) {
        ...
        const templateVariables = Array.isArray(input.metadata?.metaTemplateParameters)
            ...
            : templateVariablesFromMetadata(input.metadata?.templateVariables, templateState?.variableOrder || []);
...
function templateVariablesFromMetadata(value, order = []) {
    if (!value || typeof value !== "object" || Array.isArray(value))
        return [];
...
function adminOperationError(error, fallback) {
    ...
    if (message.startsWith("WHATSAPP_META_TEMPLATES_401") ||
        ...
        return new functions.https.HttpsError("failed-precondition", "Meta rechazó el token. Genera uno permanente con permisos de mensajería y administración.");
    }
    if (message.startsWith("WHATSAPP_META_TEMPLATES_") ||
        message.startsWith("WHATSAPP_META_TEMPLATE_CREATE_")) {
        return new functions.https.HttpsError("failed-precondition", "Meta rechazó la operación de plantillas. Revisa el nombre, el contenido y los ejemplos.");
    }
...
async function sendWhatsAppRoute(input) {
    ...
    if (!list.exists)
        return skip("lista_no_existe", { listId });
    if (list.get("activo") === false)
        return skip("lista_inactiva", { listId });
...
function isCloudProvider(provider) {
    return ["whatsapp_cloud", "cloud", "whatsappcloud"].includes(normalize(provider));
}
function metaGraphRoot(config) {
...
function metaBodyText(components) {
    if (!Array.isArray(components))
        return "";
    const body = components.find((item) => item && typeof item === "object" &&
        normalize(item.type) === "body");
    return text(body?.text);
}
function placeholderOrderFromText(templateText, allowed) {
    ...
    for (const match of templateText.matchAll(/\{([A-Za-z0-9_]+)\}/g)) {
        ...
        if (!allowedSet.has(key)) {
            throw new functions.https.HttpsError("invalid-argument", `La variable {${match[1]}} no está disponible para este mensaje.`);
        }
...
function compileMetaTemplate(definition, draftText) {
    const source = text(draftText);
    if (!source) {
        throw new functions.https.HttpsError("invalid-argument", "El borrador de la plantilla está vacío.");
    }
    const variableOrder = placeholderOrderFromText(source, definition.placeholders);
    const positions = new Map(variableOrder.map((key, index) => [key, index + 1]));
    const bodyText = source.replace(/\{([A-Za-z0-9_]+)\}/g, (_match, rawKey) => `{{${positions.get(normalize(rawKey))}}}`);
    if (bodyText.length > 1024) {
        throw new functions.https.HttpsError("invalid-argument", "Meta permite máximo 1024 caracteres en el cuerpo de esta plantilla.");
    }
    return {
        bodyText,
        variableOrder,
        examples: variableOrder.map((key) => {
            const originalKey = definition.placeholders.find((placeholder) => normalize(placeholder) === key);
            return (originalKey && definition.variableInfo[originalKey]?.sample) ||
                "Ejemplo";
        }),
    };
}
async function fetchMetaTemplateStates(config) {
    if (!isCloudProvider(config.provider)) {
        throw new functions.https.HttpsError("failed-precondition", "La consulta de plantillas requiere WhatsApp oficial (Meta Cloud API).");
    }
    if (!config.wabaId) {
        throw new functions.https.HttpsError("failed-precondition", "Falta el ID de la cuenta de WhatsApp Business (WABA).");
    }
    const url = new URL(`${metaGraphRoot(config)}/${encodeURIComponent(config.wabaId)}/message_templates`);
    ...
    for (const [key, definition] of Object.entries(MESSAGE_TEMPLATE_DEFINITIONS)) {
        ...
        const row = namedRows.find((item) => text(item.language) === definition.metaLanguage) || namedRows[0];
        if (!row)
            continue;
        const previous = config.metaTemplates[key];
        const bodyText = metaBodyText(row.components);
        const variableCount = Math.max(0, ...Array.from(bodyText.matchAll(/\{\{(\d+)\}\}/g))
...
async function persistMetaTemplateStates(empresaId, states, userId) {
...
exports.whatsappAdminGuardar = functions
    ...
    try {
        ...
        throw new functions.https.HttpsError("invalid-argument", "La URL del proveedor de WhatsApp no es válida.");
    }
...
exports.whatsappAdminSincronizarPlantillas = functions
    .region(REGION)
    .https.onCall(async (data, context) => {
    const caller = await requireWhatsAppAdmin(data, context);
    try {
        ...
        throw adminOperationError(error, "No fue posible consultar las plantillas en Meta.");
    }
});
exports.whatsappAdminEnviarPlantillaRevision = functions
    .region(REGION)
    .https.onCall(async (data, context) => {
    const caller = await requireWhatsAppAdmin(data, context);
    ...
    if (!definition) {
        throw new functions.https.HttpsError("invalid-argument", "La plantilla seleccionada no existe.");
    }
    try {
        const config = await loadRuntimeConfig(caller.empresaId);
        assertRuntimeReady(config, "admin", true);
        ...
        if (existingStates[templateKey]) {
            await persistMetaTemplateStates(caller.empresaId, existingStates, caller.userId);
            return {
                ok: true,
                alreadyExists: true,
        ...
        const configured = config.messageTemplates[templateKey];
        const compiled = compileMetaTemplate(definition, configured?.text || definition.defaultText);
        const components = [{
                type: "BODY",
        ...
        const response = await fetch(`${metaGraphRoot(config)}/${encodeURIComponent(config.wabaId)}/message_templates`, {
            method: "POST",
            headers: {
                "Content-Type": "application/json",
        ...
        throw adminOperationError(error, "No fue posible enviar la plantilla a revisión de Meta.");
    }
});
