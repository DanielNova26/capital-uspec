...
exports.isInterventoriaDeveloper = isInterventoriaDeveloper;
exports.requestNoticeRecipients = requestNoticeRecipients;
 ...
 */
function receivesInterventoriaRequestNotice(role) {
 ...
 */
function isInterventoriaDeveloper(data, empresaId) {
    ...
    return values.map(normalizeRole).some((role) => DEVELOPER_ROLES.has(role) ||
 ...
 */
function requestNoticeRecipients(roles, users, empresaId) {
    const byId = new Map(users.map((user) => [user.id, user.data]));
    const out = new Set();
    for (const role of roles) {
        if (!receivesInterventoriaRequestNotice(clean(role.rol)))
            continue;
        ...
        if (!user || userIsActiveInEmpresa(user, empresaId))
            out.add(id);
    }
 ...
 */
async function requestNoticeRecipientIds(empresaId) {
    const [roles, users] = await Promise.all([
        admin.firestore().collection(ROLES).where("empresaId", "==", empresaId).get(),
        admin.firestore().collection(USERS).select("desarrollador", "developer", "roleKey", "roleId", "role", "rol", "tipoUsuario", "empresasDetalle", "activo", "estado").get(),
    ]);
    return requestNoticeRecipients(roles.docs.map((doc) => doc.data()), users.docs.map((doc) => ({ id: doc.id, data: doc.data() })), empresaId);
}
