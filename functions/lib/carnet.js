...
const noValido = (mensaje) => pagina(`<div class="sinfoto">?</div>
 ...
 */
exports.carnetPublico = functions
    .region(REGION)
    ...
    try {
        ...
        const nombre = String(data.nombreCompleto || data.nombre || data.nombres || "").trim();
        const apellidos = String(data.apellidos ?? "").trim();
        const completo = apellidos && !nombre.includes(apellidos)
            ? `${nombre} ${apellidos}`
            : nombre;
        ...
        res.status(200).set("Content-Type", "text/html; charset=utf-8").send(pagina(`
          ${foto
