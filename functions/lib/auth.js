...
async function passwordHash(password, salt) {
    const resolvedSalt = salt ?? (0, crypto_1.randomBytes)(32);
    const key = (await scrypt(password, resolvedSalt, scryptKeyLength));
    return {
        algorithm: "scrypt-v1",
...
async function verifyHash(password, saltValue, hashValue) {
    try {
...
async function enforceAttemptLimit(context, input) {
    ...
    if (blockedUntil > Date.now()) {
        throw new functions.https.HttpsError("resource-exhausted", "Demasiados intentos. Espera unos minutos antes de volver a intentar.");
    }
}
async function recordFailure(context, input) {
    const ref = attemptRef(context, input);
...
async function clearFailures(context, input) {
    await attemptRef(context, input).delete().catch(() => undefined);
}
async function credentialFor(userDocId) {
    return db().collection(credentialsCollection).doc(credentialId(userDocId)).get();
}
async function verifyUserPassword(match, password) {
...
async function migrateCredential(match, password) {
...
async function issueToken(match) {
...
async function requireOwnUser(context) {
...
exports.authIniciarSesion = functions
    .region("us-central1")
    ...
    if (!selected) {
        ...
        throw new functions.https.HttpsError("permission-denied", "Usuario o contraseña incorrectos.");
    }
    try {
        ...
        throw new functions.https.HttpsError("unavailable", "El servicio de acceso tuvo un inconveniente temporal. Intenta nuevamente.");
    }
});
exports.authCambiarClave = functions
    .region("us-central1")
    .runWith({ timeoutSeconds: 60, memory: "512MB" })
    .https.onCall(async (data, context) => {
...
exports.authPrepararRecuperacion = functions
    .region("us-central1")
    .runWith({ timeoutSeconds: 60, memory: "512MB" })
    .https.onCall(async (data, context) => {
    const input = clean(data?.usuario, 160);
    if (!input)
        throw new functions.https.HttpsError("invalid-argument", "Ingresa tu usuario o cédula.");
...
exports.authCompletarRecuperacion = functions
    .region("us-central1")
    .runWith({ timeoutSeconds: 60, memory: "512MB" })
    .https.onCall(async (data, context) => {
    ...
    if (!userSnap.exists)
        throw new functions.https.HttpsError("not-found", "Usuario no disponible.");
