...
export const DIAN_REMITENTE_OFICIAL = "facturacionelectronica@dian.gov.co";
/** Asunto aceptado como alternativa al remitente oficial. */
export const DIAN_ASUNTO_OFICIAL = "Token Acceso DIAN";
 ...
 */
export function extraerDireccion(remitente: unknown): string {
 ...
 */
export function esCorreoTokenDian(
  remitente: unknown,
  asunto: unknown
): { aceptado: boolean; motivo: string } {
  if (extraerDireccion(remitente) === DIAN_REMITENTE_OFICIAL) {
    return { aceptado: true, motivo: "remitente" };
  }
  if (normalizar(asunto).includes(normalizar(DIAN_ASUNTO_OFICIAL))) {
    return { aceptado: true, motivo: "asunto" };
  }
  return { aceptado: false, motivo: "descartado" };
}

const ENLACE_DIAN = new RegExp(
  "https?://catalogo-vpfe\\.dian\\.gov\\.co/User/AuthToken\\?[^\\s\"'<>)\\]]+",
  "gi"
);
 ...
 */
export function extraerEnlaceDian(...partes: Array<string | false | null | undefined>): string {
  for (const parte of partes) {
    ...
    if (encontrado?.length) return encontrado[0].replace(/[.,;]+$/, "");
  }
  return "";
}

function configRef(empresaId: string): FirebaseFirestore.DocumentReference {
  return db().collection(CONFIG_COLLECTION).doc(empresaId);
}

function buzonFromData(data: FirebaseFirestore.DocumentData | undefined): BuzonConfig | null {
...
async function cargarBuzon(empresaId: string): Promise<BuzonConfig | null> {
 ...
 */
export async function estadoBuzonPublico(
  empresaId: string
): Promise<Record<string, unknown>> {
  const snap = await configRef(empresaId).get();
  const raw = (snap.data()?.buzon ?? {}) as Record<string, unknown>;
  ...
  return {
    ...
    remitenteFiltrado: DIAN_REMITENTE_OFICIAL,
    asuntoFiltrado: DIAN_ASUNTO_OFICIAL,
  };
}

function nuevoCliente(buzon: BuzonConfig, password: string): ImapFlow {
  return new ImapFlow({
    host: buzon.host,
    port: buzon.port,
    secure: true,
 ...
 */
function detalleError(error: unknown): Record<string, unknown> {
...
function camposError(error: unknown): string[] {
  const e = error as Record<string, any>;
  return [
...
function esErrorCredenciales(error: unknown): boolean {
  const e = error as Record<string, any>;
 ...
 */
async function tomarCandado(empresaId: string): Promise<boolean> {
  ...
  return db().runTransaction(async (transaction) => {
    ...
    if (
      desde instanceof admin.firestore.Timestamp &&
    ...
    transaction.set(
      ref,
      { empresaId, buzon: { sincronizandoDesde: admin.firestore.Timestamp.now() } },
      { merge: true }
    );
    return true;
  });
...
async function soltarCandado(empresaId: string): Promise<void> {
 ...
 */
async function probarConexion(buzon: BuzonConfig, password: string): Promise<void> {
 ...
 */
export async function sincronizarBuzonDian(
  empresaId: string
  ...
  if (!buzon || !buzon.appPasswordEncrypted) {
    throw new functions.https.HttpsError(
      "failed-precondition",
      "Todavía no hay un buzón conectado para esta empresa."
    );
  }
  if (!(await tomarCandado(empresaId))) {
    throw new functions.https.HttpsError(
      "aborted",
      "Ya hay una lectura del buzón en curso. Espera unos segundos."
    );
  }
  ...
  const password = decrypt(buzon.appPasswordEncrypted);
  const client = nuevoCliente(buzon, password);
  ...
  try {
    ...
    try {
      ...
      const criterio: SearchObject = {
        or: [
          { from: DIAN_REMITENTE_OFICIAL },
          { subject: DIAN_ASUNTO_OFICIAL },
        ],
      };
      ...
      for (const uid of uids) {
        const mensaje = await client.fetchOne(
          String(uid),
        ...
        try {
          const registro = await registrarTokenDianDesdeCorreo({
            empresaId,
            url,
          ...
          if (registro.created) resumen.registrados += 1;
          else resumen.duplicados += 1;
        } catch (error) {
...
export const dianBuzonEstado = functions
  .region(REGION)
...
export const dianBuzonConectar = functions
  .region(REGION)
  ...
  .https.onCall(async (data: any, context: functions.https.CallableContext) => {
    ...
    if (!email.includes("@") || email.includes(" ")) {
      throw new functions.https.HttpsError(
        "invalid-argument",
        "Indica el correo completo del buzón que recibe los tokens."
      );
    }
    if (password.length < 8) {
      throw new functions.https.HttpsError(
        "invalid-argument",
        "Pega la contraseña de aplicación generada en la cuenta Yahoo."
      );
    }
    const buzon: BuzonConfig = {
      proveedor: "yahoo",
      email,
    ...
    if (!(await tomarCandado(caller.empresaId))) {
      throw new functions.https.HttpsError(
        "aborted",
        "El buzón se está revisando en este momento. Espera unos segundos y vuelve a intentar."
      );
    }
    try {
      ...
      const camposFallo: Record<string, unknown> = {
    ...
    let resumen: ResumenSincronizacion | null = null;
    try {
...
export const dianBuzonSincronizar = functions
  .region(REGION)
...
export const dianBuzonProgramado = functions
  .region(REGION)
