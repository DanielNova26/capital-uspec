 ...
 */
export const GD_CIERRE_MOTIVOS: Record<string, string> = {
...
export const GD_CIERRE_JUSTIFICACION_MIN = 10;
export const GD_CIERRE_JUSTIFICACION_MAX = 2000;

export type GdCierreValidado = {
  motivo: string;
 ...
 */
export function gdCierreEventDetail(input: {
  cerradoPorTercero: boolean;
  cierre: GdCierreValidado;
}): string {
  const quien = input.cerradoPorTercero
    ? "Un administrador del módulo"
    : "El responsable";
  const etiqueta = GD_CIERRE_MOTIVOS[input.cierre.motivo] ?? input.cierre.motivo;
  const partes = [`${quien} marcó el proceso como terminado.`, `Motivo: ${etiqueta}.`];
  if (input.cierre.sinRespuesta) partes.push("Se cerró sin respuesta registrada.");
  if (input.cierre.justificacion) partes.push(input.cierre.justificacion);
  return partes.join(" ");
}
