...
import 'package:file_picker/file_picker.dart'; // Importado para seleccionar archivos PNG
...
class _GdFirmasScreenState extends State<GdFirmasScreen> {
  ...
  final _picker = FilePicker(); // Usar FilePicker para seleccionar archivos
  ...
  Future<void> _pickAndUpload() async {
    final result = await FilePicker.platform.pickFiles(
      ...
      allowedExtensions: ['png'], // Permitir solo archivos PNG
    );
  ...
  Future<void> _saveSignature() async {
    ...
    if (!_signatureController.isEmpty) {
      ...
      ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Por favor, dibuja tu firma o sube una imagen PNG.')));
      return;
    }
    _saveData(firmaBytes: bytes);
  }
  ...
  Future<void> _saveData({Uint8List? firmaBytes}) async {
    ...
    try {
      ...
      if (mounted) {
        ...
        _signatureController.clear(); // Limpiar después de guardar
      }
    } catch (e) {
