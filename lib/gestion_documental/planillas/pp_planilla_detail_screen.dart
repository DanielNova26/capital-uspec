...
class _PpPlanillaDetailScreenState extends State<PpPlanillaDetailScreen> {
    ...
    if (accion == 'observar' || accion == 'rechazar') {
      ...
      final confirm = await showDialog<bool>(
        ...
        builder: (ctx) => AlertDialog(
          title: Text(accion == 'observar' ? 'Agregar observación' : 'Motivo de rechazo',
          ...
          actions: [
            ...
            ElevatedButton(
              ...
              child: Text(accion == 'observar' ? 'Observar' : 'Rechazar',
      ...
      final confirm = await showDialog<bool>(
        context: context,
        builder: (ctx) => AlertDialog(
          title: Text(_accionLabel(accion), style: const TextStyle(fontFamily: kArial, fontWeight: FontWeight.w800)),
          content: Text('¿Confirmas esta acción sobre la planilla?', style: const TextStyle(fontFamily: kArial)),
          actions: [
            TextButton(onPressed: () => Navigator.pop(ctx, false), child: const Text('Cancelar')),
            ElevatedButton(
              onPressed: () => Navigator.pop(ctx, true),
              style: ElevatedButton.styleFrom(backgroundColor: GdPalette.accent),
              child: const Text('Confirmar', style: TextStyle(color: Colors.white, fontFamily: kArial)),
            ),
          ],
        ),
      );
  ...
  Widget build(BuildContext context) {
    return StreamBuilder<DocumentSnapshot<Map<String, dynamic>>>(
      stream: FirebaseFirestore.instance.collection('TBL_PP_PLANILLAS').doc(widget.planillaId).snapshots(),
        ...
        return Scaffold(
          ...
          appBar: AppBar(
            ...
            title: Text(
              ...
              style: const TextStyle(fontFamily: kArial, fontWeight: FontWeight.w800, fontSize: 15),
  ...
  Future<Uint8List> _loadPdf(String url) async {
    ...
    if (response.statusCode != 200) throw Exception('No se pudo descargar el PDF.');
    return response.bodyBytes;
  }

  Widget _buildPdfPanel(PpPlanilla planilla) {
    if (planilla.urlPdf == null) {
      return const Center(child: Text('PDF no disponible', style: TextStyle(fontFamily: kArial)));
    }
    ...
    return ListView(
      ...
      children: [
        ...
        if (planilla.datosExcel.isNotEmpty)
          _buildCard('Datos del Excel', planilla.datosExcel.entries
        ...
        _buildCard('Trazabilidad', [
          _infoRow('Cargado por', planilla.cargadoPor),
          if (planilla.revisadoPor != null) _infoRow('Revisado por', planilla.revisadoPor!),
          if (planilla.firmadoPor != null) _infoRow('Firmado por', planilla.nombreFirmante ?? planilla.firmadoPor!),
          if (planilla.firmadoEn != null) _infoRow('Firmado en', _formatTs(planilla.firmadoEn!)),
          if (planilla.cargoFirmante != null) _infoRow('Cargo', planilla.cargoFirmante!),
        ]),

        // Observaciones
        if (planilla.observaciones.isNotEmpty) ...[
          const SizedBox(height: 12),
          _buildCard('Observaciones', planilla.observaciones.map((o) => _obsRow(o)).toList()),
        ],
      ...
      child: Row(
        ...
        children: [
          ...
          Text(estado.etiqueta,
              style: TextStyle(fontFamily: kArial, fontWeight: FontWeight.w800, fontSize: 13, color: colors.$2)),
        ],
      ),
    );
  }
    ...
    return ModuleCard(
      child: Padding(
        ...
        child: Column(
          ...
          children: [
            Text(title,
                style: const TextStyle(fontFamily: kArial, fontWeight: FontWeight.w800, fontSize: 13, color: GdPalette.primary)),
  ...
  Widget _infoRow(String label, String value) => Padding(
    ...
    child: Row(
      ...
      children: [
        ...
        Expanded(
          child: Text(value,
              style: const TextStyle(fontFamily: kArial, fontSize: 13, color: GdPalette.primary)),
        ),
      ],
    ),
  );
  ...
  Widget _obsRow(Map<String, dynamic> obs) => Padding(
    ...
    child: Column(
      ...
      children: [
        Row(
          children: [
            Text(obs['nombreAutor']?.toString() ?? obs['autor']?.toString() ?? '',
                style: const TextStyle(fontFamily: kArial, fontSize: 12, fontWeight: FontWeight.w700, color: GdPalette.accent)),
            const SizedBox(width: 8),
            Text(obs['en']?.toString() ?? '',
                style: const TextStyle(fontFamily: kArial, fontSize: 11, color: GdPalette.muted)),
          ],
        ),
        const SizedBox(height: 2),
        Text(obs['texto']?.toString() ?? '',
            style: const TextStyle(fontFamily: kArial, fontSize: 13, color: GdPalette.primary)),
      ],
    ),
  );
  ...
  Widget _buildHistorial(String planillaId) {
    return StreamBuilder<List<PpFlujoEvento>>(
      ...
      builder: (context, snap) {
        ...
        return _buildCard('Historial de acciones', eventos.map((e) => Padding(
          padding: const EdgeInsets.only(bottom: 8),
          child: Row(
            crossAxisAlignment: CrossAxisAlignment.start,
            children: [
              Icon(_accionIcon(e.accion), size: 16, color: GdPalette.accent),
              const SizedBox(width: 8),
              Expanded(
                child: Column(
                  crossAxisAlignment: CrossAxisAlignment.start,
                  children: [
                    Text(PpAccionX.deString(e.accion).descripcion,
                        style: const TextStyle(fontFamily: kArial, fontSize: 13, fontWeight: FontWeight.w700)),
                    Text('${e.nombreActor ?? e.realizadoPor} · ${_formatTs(e.realizadoEn)}',
                        style: const TextStyle(fontFamily: kArial, fontSize: 11, color: GdPalette.muted)),
                    if (e.observacion != null && e.observacion!.isNotEmpty)
                      Text(e.observacion!,
                          style: const TextStyle(fontFamily: kArial, fontSize: 12, color: GdPalette.primary)),
                  ],
                ),
              ),
            ],
    ...
    return SizedBox(
      ...
      child: ElevatedButton(
        ...
        style: ElevatedButton.styleFrom(
          backgroundColor: isDestructive ? Colors.red.shade600 : GdPalette.accent,
          foregroundColor: Colors.white,
          padding: const EdgeInsets.symmetric(vertical: 14),
          shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(10)),
        ),
        child: Text(_accionLabel(accion),
            style: const TextStyle(fontFamily: kArial, fontWeight: FontWeight.w800, fontSize: 14)),
      ),
    );
  }
  ...
  List<String> _accionesDisponibles(PpPlanilla planilla) {
    ...
    if (estado == PpEstado.cargada && PpRoles.puedeEjecutar('confirmar_carga', rol)) {
      acciones.add('confirmar_carga');
    }
    if (estado == PpEstado.pendiente_validacion && PpRoles.puedeEjecutar('enviar_auditoria', rol)) {
      acciones.add('enviar_auditoria');
    }
    if (estado == PpEstado.en_revision_auditoria) {
      if (PpRoles.puedeEjecutar('observar', rol)) acciones.add('observar');
      if (PpRoles.puedeEjecutar('aprobar_auditoria', rol)) acciones.add('aprobar_auditoria');
      if (PpRoles.puedeEjecutar('rechazar_auditoria', rol)) acciones.add('rechazar');
    }
    if (estado == PpEstado.observada && PpRoles.puedeEjecutar('reenviar', rol)) {
      acciones.add('reenviar');
    }
    if (estado == PpEstado.aprobada_auditoria && PpRoles.puedeEjecutar('enviar_gerencia', rol)) {
      acciones.add('enviar_gerencia');
    }
    if (estado == PpEstado.pendiente_firma_gerencia) {
      if (PpRoles.puedeEjecutar('firmar', rol)) acciones.add('firmar');
      if (PpRoles.puedeEjecutar('rechazar_gerencia', rol)) acciones.add('rechazar');
    }
  ...
  IconData _accionIcon(String accion) => switch (accion) {
    ...
    'rechazado_auditoria' ||
    'rechazado_gerencia'  => Icons.cancel_outlined,
